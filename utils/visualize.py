...
def visualize_single(
    ...
    # Show elements with safety checks
    protein_name = f"{prefix}Protein"
    model_name = f"{prefix}Model"
    
    if protein_name in cmd.get_names('all'):
        cmd.show("sticks", protein_name)
        cmd.disable(protein_name)
        cmd.enable(protein_name)
    
    if model_name in cmd.get_names('all'):
        cmd.show("sphere", model_name)
        cmd.show("dash", model_name)
