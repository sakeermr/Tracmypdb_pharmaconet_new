...
def main(args):
    ...
    def run_pmnet_manual_center():
        ...
        if not sys.stdin.isatty():
            logging.warning("Manual center input requested in non-interactive environment - skipping")
            logging.warning("Use --ref_ligand or --center arguments for automated runs")
            return None
    ...
    if len(inform_list) == 0:
        ...
        result = run_pmnet_manual_center()
        if result is None:
            logging.error("No ligands detected and cannot run interactively. Use --ref_ligand or --center")
            return FAIL
        return SUCCESS
    ...
    if answer == "manual":
        result = run_pmnet_manual_center()
        if result is None:
            logging.error("Cannot run manual mode in non-interactive environment")
            return FAIL
        return SUCCESS
    if answer == "all":
