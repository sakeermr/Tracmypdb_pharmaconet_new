...
MODIFIED: Full TOP 50 implementation - stores 50 matches, CSV shows 50, Report shows 50
...
CHANGELOG v2.1:
- Modified to store UP TO 50 matches per molecule (was 10)
- CSV output shows TOP 50 targets (was TOP 5)
- Analysis report shows TOP 50 targets (was TOP 20)
- Output file now named: *_top50_targets.csv
- Extended organism search to 200 candidates to find 50 H/M/R matches
"""
...
class FinalOptimizedPDBScreening:
    ...
    def _screen_chemicals(self, input_df, output_file):
        ...
        for idx, row in input_df.iterrows():
            ...
            try:
                ...
                top_matches = best_matches[:100]  # Get top 100 for better organism filtering
                ...
                if top_matches:
                    ...
                    # If we don't have enough human/mouse/rat matches from top 100, extend search
                    if len(human_mouse_rat_matches) < 50:
                        # Get more matches from the full list
                        extended_matches = best_matches[100:300]  # Check next 200 matches
                        if extended_matches:
                    ...
                    # Sort human/mouse/rat matches by similarity and take top 50
                    human_mouse_rat_matches.sort(key=lambda x: x['similarity'], reverse=True)
                    final_matches = human_mouse_rat_matches[:50]  # CHANGED: Now keeps 50 matches instead of 10
                    
                    # If still no human/mouse/rat matches, use top general matches as fallback
                    if not final_matches:
                        final_matches = top_matches[:50]  # CHANGED: Fallback also uses 50 instead of 10
    ...
    def _save_comprehensive_results(self, results, output_file):
        """Save results in the requested format: CSV with top 50 targets and analysis report"""
        # Convert results to DataFrame for easier processing
        results_df = pd.DataFrame(results)
        
        # Generate CSV output file with top 50 targets
        csv_output = output_file.replace('.csv', '_top50_targets.csv')
        self._save_csv_with_top50_targets(results_df, csv_output)
        
        # Generate analysis report with top 50 targets and Tanimoto values
        ...
        self.logger.info(f"  - Top 50 targets (clean CSV): {csv_output}")
        self.logger.info(f"  - Analysis report: {report_output}")
        self.logger.info(f"  - Detailed results: {detailed_output}")
    
    def _save_csv_with_top50_targets(self, results_df, output_file):
        """Save CSV file with top 50 targets for each molecule - clean CSV format"""
        if results_df.empty:
        ...
        # Group by input chemical and get top 50 targets for each
        csv_data = []
        for chemical_name in results_df['Chemical_Name'].unique():
            ...
            if len(human_mouse_rat_data) >= 50:
                top_50_targets = human_mouse_rat_data.head(50)
            elif len(human_mouse_rat_data) > 0:
                # Take all available Human/Mouse/Rat matches even if less than 50
                top_50_targets = human_mouse_rat_data
            else:
            ...
            pdb_ids = []
            for _, row in top_50_targets.iterrows():
                pdb_ids.append(row['PDB_ID'])
            
            pdb_ids_str = ', '.join(pdb_ids)
            
            csv_data.append({
                'Plant': top_50_targets.iloc[0]['Plant'],
                'Chemical Name': top_50_targets.iloc[0]['Chemical_Name'],
                'Molecular Structure': top_50_targets.iloc[0]['Molecular_Structure'],
                'Molecule Category': top_50_targets.iloc[0]['Molecule_Category'],
    ...
    def _save_analysis_report(self, results_df, output_file):
        """Save detailed analysis report with top 50 targets and Tanimoto values"""
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            if not results_df.empty:
                ...
                for chemical_name in results_df['Chemical_Name'].unique():
                    ...
                    # Get top 50 targets
                    top_50_targets = chemical_data.head(50)
                    
                    f.write(f"MOLECULE: {top_50_targets.iloc[0]['Chemical_Name']}\n")
                    f.write(f"Plant: {top_50_targets.iloc[0]['Plant']}\n")
                    f.write(f"SMILES: {top_50_targets.iloc[0]['Molecular_Structure']}\n")
                    f.write(f"Category: {top_50_targets.iloc[0]['Molecule_Category']}\n")
                    f.write(f"Total matches found: {len(chemical_data)}\n")
                    f.write("-" * 60 + "\n")
                    
                    f.write("TOP 50 PDB TARGETS WITH TANIMOTO VALUES AND ORGANISM INFO:\n")
                    f.write(f"{'Rank':<4} {'PDB_ID':<8} {'Tanimoto':<10} {'Organism':<20} {'H/M/R':<12} {'Status':<15}\n")
                    f.write("-" * 80 + "\n")
                    
                    for i, (_, row) in enumerate(top_50_targets.iterrows(), 1):
                    ...
                    mol_similarities = top_50_targets['Similarity_Score'].astype(float)
                    f.write("\nMOLECULE STATISTICS:\n")
        ...
        self.logger.info(f"Analysis report saved with detailed Tanimoto values for TOP 50 targets")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Final Optimized PDB Ligand Screening System v2.1',
...
Output Files Generated:
  - [output_name]_top50_targets.csv    (CSV with top 50 targets per molecule)
  - [output_name]_analysis_report.txt  (Detailed analysis with top 50 targets and Tanimoto values)
  - [output_name]_detailed_results.csv (Complete results with all matches)

CHANGES IN v2.1:
  - Each molecule now stores UP TO 50 matches (upgraded from 10)
  - CSV output now shows TOP 50 targets (upgraded from TOP 5)
  - Analysis report now shows TOP 50 targets (upgraded from TOP 20)
  - Extended search range to 300 candidates to ensure 50 H/M/R matches
        """
    )
